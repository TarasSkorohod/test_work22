(function (w) {
  if (w.__pbarecommendation) return;
  ...
  function serialize(obj, prefix) {
    ...
    for (prop in obj) {
      if (obj.hasOwnProperty(prop)) {
        if (lastprop.length > 0) str += lastprop + "&";
        lastprop =
          prefix +
          encodeURIComponent(prop.toString()) +
          "=" +
          encodeURIComponent(
            obj[prop] == undefined ? "" : obj[prop].toString()
          );
      }
    }
    str += lastprop;
  ...
  w.__pbarecommendation = function (options) {
    ...
    var _defaultOpts = {
      ...
      templateprovider:
        "templateprovider.ashx" /* css and template provider url */,
      recommendation: {
        /* data: {} static data */
      ...
      lazyload: false,
      lazyloadheight: 200, // TPP - estava a 500
    };

    return new (function () {
      var self = this;
      if (typeof options.lazyload === "string")
	...
	options.lazyload = true;
      ...
      var _needPreRender = false; // controls when to get js templates (used in render and get recommendation)
      var _needRecommendation = false; // controls when to get recommendation data
      var _needRender = false; // controls when to render the template
      ...
      function getUser() {
        var cookieUser = getCookieUser();
        if (cookieUser) return cookieUser;
        else {
          try {
            var storageUser = localStorage.getItem(_cookie);
            if (storageUser) return storageUser;
            else return "";
          } catch {
            return "";
          }
      ...
      function getCookieUser() {
        ...
        if (index >= 0) {
          ...
          if (index >= 0) user = user.substr(0, user.indexOf(";"));
      ...
      function styleToObject(style) {
        return (
          (style &&
            ...
            style.split(";").reduce(function (p, c) {
              var val = c.split(":");
              if (val.length === 2) {
      ...
      function styleFromCamelCase(style) {
        return (
          (style &&
            style.constructor === String &&
            style.replace(camelRegEx, function (m) {
      ...
      function removeScript(id, tag) {
        var d = document.getElementsByTagName("script")[0];
        var scripts = Array.apply(
          [],
          d.parentElement.getElementsByTagName("script" || tag)
        ).filter(function (i) {
      ...
      function getNodeValue(node, data, name, path, index) {
        ...
        return transform &&
          (transform = getValue(self, transform) || getValue(w, transform)) &&
          transform.constructor === Function
          ? transform(getValue(data, path), path, data, index, node)
          : getValue(data, path);
      }
      ...
      function bindData(data, nodes, deep, index, ignoreempty) {
        if (data && nodes) {
          if (!nodes.length)
            /* tenho algumas dúvidas se isto funcionará para todos os casos */
            nodes = [nodes];
            ...
            for (var n = 0; n < ((elems && elems.length) || 0); n++) {
              ...
              var style =
                (node.getAttribute("style") &&
                  styleToObject(node.getAttribute("style"))) ||
                {};
              if (node.attributes && node.setAttribute) {
                for (var j = 0; j < node.attributes.length; j++) {
                  var attr = node.attributes[j];
                  if (ignoreempty && attr.value === "") continue;
                  if (attr.name.indexOf("pba-") == 0) {
                    ...
                    if (name.indexOf("attr-") == 0 && name.length > 5) {
                      ...
                      var append =
                        attr.value && attr.value.substr(0, 1) === "+";
                      var value = getNodeValue(
                        node,
                        data,
                        attrname,
                        append ? attr.value.substr(1) : attr.value,
                        index
                      );
                      if (value !== undefined) {
                        if (append && node.hasAttribute(attrname))
                          node.setAttribute(
                            attrname,
                            node.getAttribute(attrname) + " " + value
                          );
                      ...
                      if (value !== undefined) style[attrname] = value;
                      ...
                      if (!ignoreempty && !(f && f.constructor === Function)) {
                        f =
                          getValue(self, attr.value) || getValue(w, attr.value);
                      }
      ...
      function getAllNodesExceptPbaRecommendation(node) {
        var nodes = [node];

        for (var i = 0; i < node.children.length; i++) {
          var child = node.children[i];
          if (child.hasAttribute("pba-recommendation")) continue;
          nodes = nodes.concat(getAllNodesExceptPbaRecommendation(child));
        }
      ...
      function createRecommendation(templatestr, data, parent) {
        ...
        if (isNaN(nrow) || nrow <= 0 || nrow >= count) nrow = count;

        var tmplelem = document.createElement("div");
        tmplelem.innerHTML = templatestr.trim();

        bindData(
          { options: _options, response: _recommendation_response },
          getAllNodesExceptPbaRecommendation(tmplelem),
          false,
          -1,
          true
        );
        ...
        if (rec != null) {
          ...
          for (var i = 0; i < cnt; i++) {
            if (data[i].index === undefined) data[i].index = i;

            var clone = i < cnt - 1 ? rec.cloneNode(true) : rec;
            var val = null;
            (val = getValue(data[i], etemplate)) != null ||
              (val = getValue(
                { options: _options, response: _recommendation_response },
                etemplate
              )) != null ||
            ...
            if (
              val != null &&
              (val = getRecommendationCustomTemplate(tmplelem, val)) != null
            ) {
              ...
              while (val.firstChild) clone.appendChild(val.firstChild);
            } else {
        ...
        classes.push.apply(
          classes,
          (container.getAttribute("class") || "").split(" ")
        );
      ...
      function getRecommendation() {
        if (_options.recommendation) {
          if (_options.recommendation.data) {
            ...
            _needRecommendation = false;
          } else if (_options.recommendation.url && !_recLoading) {
            _recLoading = true;
            var url = getRecommendationUrl();
            _on["changeRecommendationUrl"] &&
              _on["changeRecommendationUrl"].forEach(function (f) {
                url = f(url, self);
              });
            ...
            callAjax(url, function (data) {
              ...
              _recommendation_data =
                (json.recommendation &&
                  json.recommendation.results &&
                  json.recommendation.results.result) ||
                {};
              _on["recommendationData"] &&
                _on["recommendationData"].forEach(function (f) {
                  _recommendation_data = f(_recommendation_data, self);
                });
              _needRecommendation = false;
              _recLoading = false;
            });
      ...
      function getTemplate() {
        if (_options.template) {
          if (_options.template.id) {
            ...
            if (template) _recommendation_template = template.innerHTML;
          } else if (_options.template.name) {
            var url =
              _options.templateprovider +
              "?type=html&name=" +
              _options.template.name;
            callAjax(url, function (data) {
      ...
      function getCss() {
        _cssReady = 0;
        if (_options.css) {
          var csslist = Array.isArray(_options.css)
            ? _options.css
            : [_options.css];
      ...
      function getJS() {
        ...
        if (_options.js) {
          ...
          _needPreRender = jslist.length > 0;
          ...
          jslist.forEach(function (opt) {
            if (opt.name || opt.url) {
              ...
              } else if (opt.name) templateproviderscripts.push(opt.name);
            }
          });
          ...
          if (templateproviderscripts.length) {
            var url =
              ...
              _id +
              "&name=" +
              templateproviderscripts.join("&name=");
      ...
      if (_options.lazyload) {
        function lazy() {
          ...
          if (_needRender) setTimeout(lazy, 100);
        }
        setTimeout(lazy, 100);
      } else {
        getTemplate();
      ...
      setInterval(function () {
        if (_options.lazyload && !_lazyloaded) return;

        // first get js
        if (_needPreRender && _pollJS.length == 0 && !_jsLoading) {
          _on["preRender"] &&
            _on["preRender"].forEach(function (f) {
              f(self);
            });
          _needPreRender = false;
        }

        if (_needRecommendation && !_needPreRender) {
          getRecommendation();
        }
        ...
        if (
          ...
          if (_recommendation_element && isDomNode(_recommendation_element))
            createRecommendation(
              _recommendation_template,
              _recommendation_data,
              _recommendation_element
            );

          _needRender = false;
          _on["postRender"] &&
            _on["postRender"].forEach(function (f) {
              f(self);
            });
        }
      }, 100);
      ...
      this.on = function (event, callback) {
        if (
          ...
          if (_on[event].indexOf(callback) < 0) _on[event].push(callback);
        }
        return this;
      };
      ...
      this.log = function () {
        ...
        if (id && id.indexOf("#") > 0) id = id.substr(0, id.indexOf("#"));
        if (id && id.indexOf("?") > 0) id = id.substr(0, id.indexOf("?"));
        ...
        var opt = concat(
          {
            ...
            ////// HACK HACK HACK
            site_id: getSiteID(),
            abtest: _options.abtest,
            abtestid: _options.abtestid,
            from_id: id,
            rec_pos: _options.rec_pos,
            config: getMethod(),
      ...
      this.destroy = function () {
        ...
        if (_recommendation_element) _recommendation_element.innerHTML = "";
